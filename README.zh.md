...
1. **GUI 中的本地更新模式与多基础版本支持** — 直接在用户电脑的安装程序图形界面中运行更新流程，支持更新用户自身的存档文件并兼容多个目标版本（**SP Football Life 2026**、**vanilla eFootball PES 2021** 和 **UML**）。
2. **集成 Pes Retro Stats 转换器与 OVR 计算器** — 完整集成属性转换与基于位置加权的综合评分（OVR）计算引擎到流水线与工作流中（当前 `tools/` 中的脚本为初始原型）。
