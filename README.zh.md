...
1. **GUI 中的本地更新模式** — 四步向导现已可更新用户选择的、采用已验证
   通用布局的 `EDIT00000000` 存档，并提供 **Fast** 或 **Deep** 覆盖。本地
   更新资格不受存档的 SPFL/PES/UML 标签影响；存档会在修改前检查并在原位置
   备份，随后仅替换为经过原子验证的结果。本功能不会发布新的远程资源：可下载
   版本仍仅面向已验证的 FL26/SPFL 目标。
