...
def test_edit_file_reads_strong_foot_from_save_player_record():
    from tests.test_editor import _build_mock_data

    edit_file = EditFile()
    edit_file.load_bytes(
        _build_mock_data(
            num_players=1,
            num_teams=0,
            num_team_player=0,
            num_game_plans=0,
            player_entries=[(999999, "Left-footed CB")],
        )
    )
    edit_file._data[edit_file.player_start + PE_STRONG_FOOT_BYTE] = 1 << 5

    assert edit_file.get_player_preferred_foot(999999) == "L"
...
def test_integrity_rejects_missing_goalkeeper_position_code_in_each_phase():
    from tests.test_editor import _build_mock_data
    ...
    for preset_offset in GP_POSITION_PRESETS:
        for phase_offset in GP_POSITION_PHASE_OFFSETS:
            ...
            edit_file._data[position_offset : position_offset + 11] = bytes([1] * 11)

    report = edit_file.validate_integrity()

    assert report["valid"] is False
    assert report["metrics"]["checked_position_arrays"] == 9
    assert sum(
        "expected exactly 1" in error for error in report["errors"]
    ) == 9


def test_repair_game_plan_restores_missing_gk_arrays_and_starter():
    from tests.test_editor import _build_mock_data

    roster_ids = [108380, 165712, 131304] + list(range(200000, 200037))
    starters = [0, 7, 3, 11, 8, 13, 16, 26, 22, 21, 17]
    bench = [2, 1] + [
        slot
        for slot in range(40)
        if slot not in starters and slot not in {1, 2}
    ]
    data = _build_mock_data(
        num_players=40,
        num_teams=1,
        num_team_player=1,
        num_game_plans=1,
        team_player_entries=[(122, roster_ids, list(range(1, 41)))],
        league_team_ids=[122],
    )
    edit_file = EditFile()
    edit_file.load_bytes(data)
    edit_file._player_cache = {
        108380: PlayerInfo(108380, "Pedraza", position="LB"),
        165712: PlayerInfo(165712, "Motta", position="GK"),
        131304: PlayerInfo(131304, "Furlanetto", position="GK"),
    }
    game_plan_offset = edit_file.game_plan_start
    lineup_offset = game_plan_offset + GP_LINEUP
    edit_file._data[lineup_offset : lineup_offset + 40] = bytes(starters + bench)
    broken_positions = bytes([2, 1, 1, 3, 2, 4, 5, 5, 10, 9, 12])
    for preset_offset in GP_POSITION_PRESETS:
        for phase_offset in GP_POSITION_PHASE_OFFSETS:
            position_offset = game_plan_offset + preset_offset + phase_offset
            edit_file._data[position_offset : position_offset + 11] = broken_positions

    metrics = edit_file.repair_game_plans()

    roster = edit_file.get_team_roster(122)
    assert roster is not None
    lineup = list(edit_file._data[lineup_offset : lineup_offset + 40])
    assert roster.player_ids[lineup[0]] in {165712, 131304}
    assert metrics["repaired_goalkeeper_roles"] == 1
    assert metrics["repaired_position_bytes"] == 9
    for preset_offset in GP_POSITION_PRESETS:
        for phase_offset in GP_POSITION_PHASE_OFFSETS:
            position_offset = game_plan_offset + preset_offset + phase_offset
            positions = list(edit_file._data[position_offset : position_offset + 11])
...
def test_position_aware_gameplan_repair_promotes_exact_roles():
    from tests.test_editor import _build_mock_data
    ...
    metrics = edit_file.repair_game_plans(
        preferred_starters={101: (1011, 1012)},
        position_overrides={101: {1011: "RW", 1012: "LW"}},
    ...
    starter_ids = [roster.player_ids[slot] for slot in lineup]

    assert 1011 in starter_ids
    assert 1012 in starter_ids
    assert 1007 not in starter_ids
...
def test_cb_roles_prefer_matching_strong_foot_by_side():
    from tests.test_editor import _build_mock_data

    player_ids = list(range(4000, 4012))
    positions = [
        "GK",
        "CB",
        "CB",
    ...
    data = _build_mock_data(
        ...
        player_entries=[(player_id, f"Player {player_id}") for player_id in player_ids],
        league_team_ids=[101],
    )
    edit_file = EditFile()
    edit_file.load_bytes(data)
    for index, foot in enumerate(feet):
        if foot == "L":
            edit_file._data[
                edit_file.player_start + index * PLAYER_TOTAL_SIZE + PE_STRONG_FOOT_BYTE
            ] |= 1 << 5
    edit_file.attach_playerbin(
        PlayerBinDatabase(
            {
    ...
    game_plan_offset = edit_file.game_plan_start
    target_positions = [0, 1, 1, 1, 3, 2, 4, 5, 8, 10, 12]
    for preset_offset in GP_POSITION_PRESETS:
        for phase_offset in GP_POSITION_PHASE_OFFSETS:
            edit_file._data[
                game_plan_offset + preset_offset + phase_offset :
    ...
    assert starter_ids[1:4] == [4001, 4003, 4002]
    two_cb_positions = [0, 1, 1, 3, 2, 4, 5, 8, 10, 9, 12]
    for preset_offset in GP_POSITION_PRESETS:
        for phase_offset in GP_POSITION_PHASE_OFFSETS:
            edit_file._data[
                game_plan_offset + preset_offset + phase_offset :
                game_plan_offset + preset_offset + phase_offset + 11
            ] = bytes(two_cb_positions)
    edit_file._data[
        game_plan_offset + GP_LINEUP :
        game_plan_offset + GP_LINEUP + 11
    ] = bytes([0, 2, 3, 4, 5, 6, 7, 8, 11, 9, 10])
    ...
    assert starter_ids[1:3] == [4003, 4002]


def test_position_aware_repair_rejects_cross_line_and_opposite_wing_players():
    from tests.test_editor import _build_mock_data
    ...
    lineup = [0, 1, 2, 3, 4, 5, 6, 11, 12, 7, 10, 8, 9]
    edit_file._data[
        game_plan_offset + GP_LINEUP :
    ...
    repaired_positions = [
        positions[roster.player_ids[slot] - 2000]
        for slot in repaired_lineup
    ]

    assert repaired_positions[7] == "RWF"
    assert repaired_positions[8] == "LWF"
    assert repaired_positions[9] == "AMF"
...
def test_repair_game_plans_preserves_cross_line_tactical_positions():
        ...
        PlayerBinDatabase(
            {
                ...
                1001: PlayerBinRecord(1001, "Starting LB", 24, "LB", 0),
                1002: PlayerBinRecord(1002, "Starting RB", 24, "RB", 0),
            }
        )
    )
    game_plan_offset = edit_file.game_plan_start
    ...
    for preset_offset in GP_POSITION_PRESETS:
        for phase_offset in GP_POSITION_PHASE_OFFSETS:
            position_offset = game_plan_offset + preset_offset + phase_offset
            assert edit_file._data[position_offset : position_offset + 3] == bytes(
                [0, 8, 4]
            )


def test_repair_game_plans_preserves_tactical_winger_variants():
    from tests.test_editor import _build_mock_data
    ...
    lineup_offset = game_plan_offset + GP_LINEUP
    edit_file._data[lineup_offset : lineup_offset + 40] = bytes(range(40))
    expected_positions = [0, 1, 1, 1, 1, 1, 1, 7, 6, 12, 12]
    for preset_offset in GP_POSITION_PRESETS:
        for phase_offset in GP_POSITION_PHASE_OFFSETS:
            position_offset = game_plan_offset + preset_offset + phase_offset
            edit_file._data[position_offset : position_offset + 11] = bytes(
    ...
    for preset_offset in GP_POSITION_PRESETS:
        for phase_offset in GP_POSITION_PHASE_OFFSETS:
            position_offset = game_plan_offset + preset_offset + phase_offset
            assert edit_file._data[position_offset : position_offset + 11] == bytes(
                expected_positions
            )
...
def test_added_player_preserves_tactical_position_for_changed_game_plan_role(
    player_id: int,
    native_position: str,
    ...
    game_plan_offset = edit_file.game_plan_start
    lineup_offset = game_plan_offset + GP_LINEUP
    tactical_position_code = POSITION_NAMES.index("CF")
    for preset_offset in GP_POSITION_PRESETS:
        for phase_offset in GP_POSITION_PHASE_OFFSETS:
            edit_file._data[
                game_plan_offset + preset_offset + phase_offset + 10
            ] = tactical_position_code
    ...
    expected_position_code = (
        POSITION_NAMES.index("GK")
        if expected_role == 0
        else tactical_position_code
    )
    for preset_offset in GP_POSITION_PRESETS:
        for phase_offset in GP_POSITION_PHASE_OFFSETS:
            assert edit_file._data[
...
def test_removal_preserves_copied_player_tactical_position():
    from tests.test_editor import _build_mock_data
...
def test_move_preserves_copied_player_tactical_position_after_source_compaction():
    from tests.test_editor import _build_mock_data
...
def test_removal_preserves_promoted_player_tactical_position():
    from tests.test_editor import _build_mock_data
