...
from scraper.models import CaptainUpdate, ManagerUpdate, MatchedTransfer, Transfer
...
def test_manager_mapping_loader_accepts_integer_id_object(tmp_path):
    import run

    mapping = tmp_path / "managers.json"
    mapping.write_text('{"101": 900, "102": 0}', encoding="utf-8")

    assert run._load_manager_mapping(mapping) == {101: 900, 102: 0}


def test_manager_mapping_loader_rejects_non_integer_target(tmp_path):
    import run

    mapping = tmp_path / "managers.json"
    mapping.write_text('{"101": "Coach One"}', encoding="utf-8")

    with pytest.raises(ValueError, match="must be an integer"):
        run._load_manager_mapping(mapping)



def test_manager_update_dry_run_reports_changes_without_encrypting(
    monkeypatch, tmp_path, capsys
):
    import config
    import run
    from tests.test_editor import _build_mock_data
    monkeypatch.setattr(config, "TRANSFER_LOG_FILE", tmp_path / "manager-log.jsonl")

    edit_path = tmp_path / "EDIT00000000"
    edit_path.write_bytes(b"encrypted-edit")
    mapping_path = tmp_path / "managers.json"
    mapping_path.write_text('{"101": 901}', encoding="utf-8")

    decrypted_dir = tmp_path / "decrypted"
    decrypted_dir.mkdir()
    (decrypted_dir / "data.dat").write_bytes(
        _build_mock_data(
            num_players=32,
            num_teams=2,
            num_managers=2,
    ...
    monkeypatch.setattr(run.crypto, "decrypt", lambda _path: decrypted_dir)
    monkeypatch.setattr(
        run.crypto,
        "encrypt",
        lambda *_args: pytest.fail("dry-run must not encrypt"),
    )

    run.cmd_manager_update(
        Namespace(
            edit_file=edit_path,
    ...
    assert "Alpha FC (101)" in output
    assert "Coach Two (901)" in output
    assert "1 manager changes would be applied" in output
    assert not (tmp_path / "updated" / "EDIT00000000").exists()



def test_manager_update_auto_matches_scraped_manager_and_logs(
    monkeypatch, tmp_path, capsys
):
    import config
    import run
        ...
        lambda **_kwargs: SimpleNamespace(
            manager_updates=(
                ManagerUpdate(
                    club_name="Alpha FC",
                    team_id_fotmob=42,
                    manager_name="Coach Two",
                    manager_id_fotmob=777,
    ...
    assert "Coach Two (901)" in output
    assert "1 manager changes would be applied" in output
    entries = run.transfer_logger.read_log()
    assert entries[0]["transfer_type"] == "manager_update"
    assert entries[0]["source_urls"] == ["https://example.test/teams/42"]
